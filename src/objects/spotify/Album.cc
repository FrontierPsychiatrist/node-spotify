...
std::string Album::coverLink() {
  std::string link;
  if(sp_album_is_loaded(album)) {
    char linkChar[256];
    sp_link* spLink = sp_link_create_from_album_cover(album, SP_IMAGE_SIZE_LARGE);
    sp_link_as_string(spLink, linkChar, 256);
    link = std::string(linkChar);
    sp_link_release(spLink);
  }
...
std::string Album::coverBase64() {
  std::string cover;
  if(sp_album_is_loaded(album)) {
    const byte* coverId = sp_album_cover(album, SP_IMAGE_SIZE_LARGE);
    if(coverId != nullptr) {
